...
export default function DetailedReportsPage() {
  ...
  const maxGrad = Math.max(
    ...data.graduationDistribution.map((d) => d.count),
    1,
  );
  const totalEmployment =
    data.employmentDistribution.reduce((a, d) => a + d.count, 0) || 1;
  const totalAlumniDept =
    data.alumniByDepartment.reduce((a, d) => a + d.count, 0) || 1;
  const totalStudentDept =
    data.studentsByDepartment.reduce((a, d) => a + d.count, 0) || 1;
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {[
          ...
          {
            label: "Akademisyen",
            value: overview.totalAcademicians,
            icon: Award,
            color: "bg-violet-500/10 text-violet-600",
          },
          {
            label: "Bölüm Başkanı",
            value: overview.totalHeadOfDepts,
            icon: UserCog,
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {[
          ...
          <div
            key={item.label}
            className="bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/15 rounded-xl p-5"
          >
                ...
                <div key={item.department}>
                  ...
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(item.count / totalAlumniDept) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div key={item.department}>
                  ...
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(item.count / totalStudentDept) * 100}%`,
                      }}
                    />
                  </div>
                </div>
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Mezunların çalıştığı sektörler (en çok 10)
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          },
          {
            ...
            sub: `${overview.upcomingEvents} yaklaşan`,
            icon: TrendingUp,
          ...
          return (
            ...
            >
              <div
                className={`w-9 h-9 rounded-lg flex items-center justify-center ${card.color} mb-3`}
              >
                <Icon className="w-4.5 h-4.5" />
              </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="font-bold text-slate-900">Detaylı Kırılım Raporu</h2>
            <p className="text-xs text-slate-500">
              Bölüm / İl / Çalışma durumu bazlı derinlemesine analiz
            </p>
          </div>
        </div>
        ...
        {data.departmentDetails?.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-8">
            Henüz detaylı veri yok
          </p>
        ) : (
          <div className="space-y-4">
            {data.departmentDetails?.map((deptItem) => (
              <div
              ...
              >
                <button
                  onClick={() =>
                    setExpandedDept(
                      expandedDept === deptItem.department
                        ? null
                        : deptItem.department,
                    )
                  }
                ...
                >
                  ...
                  <span className="bg-white border border-slate-200 text-slate-700 text-xs font-bold px-2 py-1 rounded-md">
                    {deptItem.count} Mezun
                  </span>
                </button>

                {expandedDept === deptItem.department && (
                  <div className="p-4 bg-white border-t border-slate-200 space-y-3">
                    {deptItem.cities.map((cityItem) => (
                      <div
                        key={cityItem.city}
                      ...
                      >
                        ...
                        {expandedCity === cityItem.city && (
                          <div className="p-4 bg-slate-50 border-t border-slate-100 pl-10">
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                              {cityItem.statuses.map((statusItem) => (
                                <div
                                  key={statusItem.status}
                                  className="bg-white border border-slate-200 p-3 rounded-lg flex items-center justify-between"
                                >
                                  <div className="flex items-center gap-2">
                                    <div
                                      className={`w-2 h-2 rounded-full ${employmentColors[statusItem.status] || "bg-slate-400"}`}
                                    />
                                    <span className="text-xs font-medium text-slate-600">
                                      {employmentLabels[statusItem.status] ||
                                        statusItem.status}
                                    </span>
                                  </div>
