...
import {
    Clock, CheckCircle, XCircle, User, GraduationCap,
    Briefcase, BookOpen, UserCog, Filter, Phone,
...
const roleConfig: Record<string, { label: string; icon: React.ElementType; color: string }> = {
    ALUMNI: { label: "Mezun", icon: Briefcase, color: "bg-blue-500/10 text-blue-600" },
    STUDENT: { label: "Öğrenci", icon: GraduationCap, color: "bg-green-500/10 text-green-600" },
    ACADEMICIAN: { label: "Akademisyen", icon: BookOpen, color: "bg-violet-500/10 text-violet-600" },
    HEAD_OF_DEPARTMENT: { label: "Bölüm Başkanı", icon: UserCog, color: "bg-amber-500/10 text-amber-600" },
};
...
export default function AdminApprovalsPage() {
    ...
    const handleAction = async (userId: string, action: "APPROVE" | "REJECT") => {
        ...
        try {
            ...
            if (!res.ok) throw new Error();
            toast.success(action === "APPROVE" ? "Üye onaylandı ✓" : "Üye reddedildi");
    ...
    return (
        <div className="p-6 lg:p-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Üye Onay Yönetimi</h1>
                ...
                {statusFilter === "PENDING" && users.length > 0 && (
                    <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl">
                        ...
                        <span className="text-sm font-bold text-amber-700">{users.length} bekleyen başvuru</span>
                    </div>
                )}
            </div>
                    ...
                    <div className="flex rounded-xl overflow-hidden border border-slate-200">
                        {[
                            ...
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-semibold transition-all ${statusFilter === opt.value
            ...
            {users.length === 0 ? (
                <div className="bg-white border border-slate-200 rounded-xl p-16 text-center">
                    ...
                    <h3 className="font-semibold text-slate-700 mb-1">Bekleyen başvuru yok</h3>
                    <p className="text-sm text-slate-400">Şu an seçili kriterlere uyan başvuru bulunmuyor.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {users.map((user) => {
                        ...
                        return (
                            <div key={user.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-md transition-all">
                                <div className="p-6">
                                    <div className="flex items-start gap-4">
                                        {/* Avatar */}
                                        {user.image ? (
                                            <Image src={user.image} alt={user.firstName} width={52} height={52} className="rounded-xl object-cover flex-shrink-0" />
                                        ) : (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-3">
                                                ...
                                                {role && (
                                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold flex-shrink-0 ${role.color}`}>
                                                    ...
                                                    <>
                                                        ...
                                                        {user.alumni.graduationYear && (
                                                            <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                                                ...
                                                                <span>Mezuniyet: {user.alumni.graduationYear}</span>
                                                            </div>
                                                        )}
                                                        ...
                                                        {user.alumni.referenceTeacher && (
                                                            <div className="flex items-center gap-1.5 text-xs text-slate-600 font-semibold col-span-2">
                                                                ...
                                                                <span>Referans: {user.alumni.referenceTeacher}</span>
                                                            </div>
                                                        )}
                                                        {user.alumni.employmentStatus && (
                                                            <div className="flex items-center gap-1.5 text-xs text-slate-500 col-span-2">
                                                                <Briefcase className="w-3.5 h-3.5 flex-shrink-0" />
                                                                <span>{employmentLabels[user.alumni.employmentStatus] ?? user.alumni.employmentStatus}
                                                                    {user.alumni.employmentSector && ` — ${user.alumni.employmentSector}`}
                                                                </span>
                                                            </div>
                                                        )}
                                                    </>
                                                ...
                                                {user.student && (
                                                    <>
                                                        ...
                                                        {user.student.referenceTeacher && (
                                                            <div className="flex items-center gap-1.5 text-xs text-slate-600 font-semibold col-span-2">
                                                                <User className="w-3.5 h-3.5 flex-shrink-0 text-primary" />
                                                                <span>Referans: {user.student.referenceTeacher}</span>
                                                            </div>
                                                        )}
                                                    </>
                                                )}
