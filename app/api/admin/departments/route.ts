...
export async function GET(req: Request) {
    try {
        ...
        const where = includeInactive && isAdmin(session?.user?.role)
            ? {}
            : { isActive: true };
...
export async function POST(req: Request) {
    try {
        ...
        if (!name?.trim()) {
            return NextResponse.json({ error: "Bölüm adı zorunludur" }, { status: 400 });
        }

        const existing = await prisma.department.findUnique({ where: { name: name.trim() } });
        if (existing) {
            return NextResponse.json({ error: "Bu isimde bölüm zaten mevcut" }, { status: 400 });
        }
