...
export async function POST(req: Request) {
    try {
        ...
        if (!name?.trim() || !departmentId) {
            return NextResponse.json({ error: "Program adı ve bölüm zorunludur" }, { status: 400 });
        }
        ...
        if (error.code === "P2002") {
            return NextResponse.json({ error: "Bu bölümde aynı isimde program zaten var" }, { status: 400 });
        }
