...
const reportSchema = z.object({
  reportedId: z
    .string()
    .min(1, "Rapor edilecek içerik/kullanıcı ID'si gereklidir."),
  type: z.nativeEnum(ReportType),
  reason: z
    .string()
    .min(
      10,
      "Lütfen şikayet nedeninizi daha detaylı açıklayın (En az 10 karakter).",
    )
    .max(500),
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json(
      {
        message:
          "Şikayetiniz başarıyla iletildi. Moderatörlerimiz inceleyecektir.",
      },
      { status: 201 },
    );
  } catch (error) {
